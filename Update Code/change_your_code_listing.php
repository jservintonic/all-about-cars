...
<?php
  ...
  /* USED ONLY IF REDIRECTED TO LISTING AFTER SUBMITTED BID
  if($_POST["submit"] == "YES") {
    $new_bid_price = $_SESSION["new_price"];
    $sql = "INSERT INTO bids (`buyerID`, `itemID`, `bidPrice`, `bidDate`)
            VALUES ($buyerID, $item_id, $new_bid_price, now());";
    $conn->query($sql);
  ...
  */
  ...
  // Get item records from database
  ...
  $sql = "SELECT MAX(bidPrice) AS bidPrice
          ...
          //AND bidDate = (SELECT MAX(bidDate) 
          //               FROM bids);";
  $result = mysqli_query($conn, $sql);
  ...
  // Session for place_bid.php and watch_listing.php
  ...
  // Time vars (What about timezone??)
  $timezone = date_default_timezone_set('Europe/London');
  $listing_end = $listing["endDate"];
  // TEST $end_time = new DateTime('2020-11-02T00:00:00');
  $end_time = new DateTime($listing_end);
  // TEST $end_time = new DateTime('2020-11-02 00:00:00');
...
?>
  ...
  <!-- if not logged in, redirect to login when try adding to watchlist -->
  <?php if ($has_session == false) : ?>
    <style type="text/css">#watch_nowatch{display:none;}</style>
    <!-- dummy button to login when clicked -->
        ...
        <div id="demo" class="carousel slide" data-ride="carousel" style="width:100%; height: 500px !important; margin-bottom:30px;">
          ...
          <div class="carousel-inner">
            <div class="carousel-item active">
              ...
              <!--<img style=" width:100%; height: 500px !important;" src="images/la.jpg" alt="Los Angeles">-->
            </div>
...
<?php if ($now > $end_time): ?>
     ...
     <?php 
      ...
      $sql = "SELECT COUNT(p.purchaseID) AS pCount
              FROM purchase p
              INNER JOIN bids b ON p.bidID = b.bidID
...
<?php else: ?>
    ...
    <!-- if no session: urge user to login when submitting a bid -->
    <?php else : ?>
      <form>
        <div class="input-group">
          <div class="input-group-prepend">
